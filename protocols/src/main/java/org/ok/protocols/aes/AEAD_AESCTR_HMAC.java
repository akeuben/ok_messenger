...
public class AEAD_AESCTR_HMAC {
    ...
    public Block encrypt(byte[] messageKey, Block plaintext, Block associatedData) {
        ...
        Block[] res = new KDF().kdf_ck(messageKey);

        Block nonceBlock = new Block(nonce);

        Block ciphertext = aes.encrypt(new AESKey(res[0].getData()), plaintext, associatedData);

        Block hmac = new HMAC().encode(Block.concat(nonceBlock, ciphertext, associatedData), res[1]);
    ...
    public Block decrypt(byte[] messageKey, Block encryptedData, Block associatedData) {
        ...
        Block expectedHmac = new HMAC().encode(Block.concat(nonce, ciphertext, associatedData), res[1]);
        if(!recievedHmac.equals(expectedHmac)) {
            throw new RuntimeException("Failed to verify message through HMAC");
        }

        return aes.decrypt(new AESKey(res[0].getData()), ciphertext, nonce);
    }
}
