...
import org.ok.math.GF256;
import org.ok.protocols.Block;

public abstract class AES {

    static byte[] encryptionSTable = new byte[] {
    ...
    static byte[] decryptionSTable = new byte[] {
    ...
    static Block Dec_AddRoundKey(Block data, Block roundKey) {
        assert data.getSizeBits() == 128;
        return data.xor(roundKey);
    }
    ...
    static Block Dec_SubBytes(Block data) {
        assert data.getSizeBits() == 128;
        return data.byteWiseOperation((b -> decryptionSTable[((int) b) & 0xFF]));
    }
    ...
    static Byte[] shiftRowRight(Byte[] row, int count) {
        Byte[] newRow = new Byte[row.length];
    ...
    static Block Dec_ShiftRows(Block data) {
        assert data.getSizeBits() == 128;
        return data.rowWiseOperation(AES::shiftRowRight);
    }

    // MixColumns transformation (column-major order)
    ...
    static Block Dec_MixColumns(Block data) {
        assert data.getSizeBits() == 128;
        byte[] state = data.getData();
        ...
        for (int i = 0; i < 16; i += 4) {  // Process each column separately
            ...
            state[i]     = (byte) (GF256.multiply(s0, 14) ^ GF256.multiply(s1, 11) ^ GF256.multiply(s2, 13) ^ GF256.multiply(s3, 9));
            state[i + 1] = (byte) (GF256.multiply(s0, 9)  ^ GF256.multiply(s1, 14) ^ GF256.multiply(s2, 11) ^ GF256.multiply(s3, 13));
            state[i + 2] = (byte) (GF256.multiply(s0, 13) ^ GF256.multiply(s1, 9)  ^ GF256.multiply(s2, 14) ^ GF256.multiply(s3, 11));
            state[i + 3] = (byte) (GF256.multiply(s0, 11) ^ GF256.multiply(s1, 13) ^ GF256.multiply(s2, 9)  ^ GF256.multiply(s3, 14));
        }
