...
%cite github
These constants are used in most of the other existing SHA-256 implementations and have not been proven to be insecure, hence we have chosen them for our implementation.
...
\end{lstlisting}
Implementing SHA-256, HMAC, HKDF, and KDFs allowed us to appreciate how the simplicity of manipulating bits can produce algorithms that ensure our data is secure from adversaries. Overall we were able to see how the algorithms can be used to ensure cryptographic security with other algorithms that depend on it, making a dependency tree that shows how one cryptographically secure algorithm can be used to build other cryptographically secure algorithms for different purposes. 